...
from Lesson1.database_setup import Restaurant, Base, MenuItem
...
""" Additional Lesson: query commands for SQLAlchemy - http://docs.sqlalchemy.org/en/rel_0_9/orm/query.html """
...
""" Additional Lesson: CRUD Updates """
#querying
...
UrbanVeggieBurger = session.query(MenuItem).filter_by(id=8).one() #just one
print(UrbanVeggieBurger.price)

# 2. reset values
UrbanVeggieBurger.price = '$2.99' # 할인!
session.add(UrbanVeggieBurger)# 3. add to session
session.commit() # 4. session.commit()
# > Urban Burger의 veggieBurger 가격이 떨어짐

# resetting all veggie burger price
for veggieBurger in veggieBurgers:
